...
function airprops(t, p, phi) {
    ...
    result["Moisture (PPMv)"] = result["Sat Water Pressure (psia)"]/(p - result["Sat Water Pressure (psia)"]) * 1e6;
    result["Moisture (PPMm)"] = 18.01528*result["Sat Water Pressure (psia)"]/(28.9647*(p - result["Sat Water Pressure (psia)"])) * 1e6;
    return result;
}
